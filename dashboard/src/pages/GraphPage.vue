...
<template>
  <div style="display: flex; flex-direction: column; gap: 1rem;">
    ...
    <div v-if="store.nodes.length > 0" style="display: flex; gap: 1rem;">
      <div :style="{ flex: store.selectedNode ? '1 1 65%' : '1 1 100%', minWidth: 0, transition: 'flex 0.3s' }">
        <GraphVisualization />
      </div>
      <div v-if="store.selectedNode" style="flex: 0 0 320px; max-height: 650px; overflow-y: auto;">
        <NodeInspectorPanel />
      </div>
    </div>
