...
import type { LayoutMode, EdgeTypeFilter } from '@/types/graph'
...
<template>
  <div style="display: flex; flex-direction: column; gap: 0.75rem;">
    ...
    <div v-if="store.nodes.length > 0" style="display: flex; gap: 1rem; align-items: center; flex-wrap: wrap; font-size: 0.85rem;">
      <div style="display: flex; align-items: center; gap: 0.5rem;">
        <span style="color: var(--p-text-muted-color);">Layout:</span>
        <SelectButton v-model="store.layoutMode" :options="layoutOptions" optionLabel="label" optionValue="value" :allowEmpty="false" size="small" />
      </div>
      <div style="display: flex; align-items: center; gap: 0.5rem;">
        <span style="color: var(--p-text-muted-color);">Edge type:</span>
        <Select v-model="store.edgeTypeFilter" :options="edgeFilterOptions" optionLabel="label" optionValue="value" style="width: 9rem;" size="small" />
      </div>
      <div style="display: flex; align-items: center; gap: 0.5rem; flex: 1; min-width: 12rem;">
        <span style="color: var(--p-text-muted-color);">Filter:</span>
        <InputText v-model="store.filePatternFilter" placeholder="File pattern..." size="small" style="flex: 1;" />
      </div>
