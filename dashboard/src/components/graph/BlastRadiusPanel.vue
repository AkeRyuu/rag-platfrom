...
const treeChartOption = computed(() => {
  if (!store.blastRadius || store.blastRadius.affectedFiles.length === 0) return {}
    ...
    tooltip: {
      formatter: (params: any) => {
        if (params.dataType === 'node') {
          const hop = store.blastRadius?.affectedFiles.find(f => f.file === params.data.id)?.hop
...
<template>
  <Card v-if="store.searchFile">
    ...
    <template #content>
      ...
      <!-- List View -->
      <div v-else-if="store.blastViewMode === 'list'" style="display: flex; flex-direction: column; gap: 0.75rem;">
        <div style="font-size: 0.875rem;">
          <b>{{ store.blastRadius.totalAffected }}</b> files affected
        </div>
        <div v-for="[hop, files] in groupedByHop" :key="hop">
          <div style="display: flex; align-items: center; gap: 0.5rem; margin-bottom: 0.25rem;">
            <Tag :value="`Hop ${hop}`" :style="{ background: HOP_COLORS[Math.min(hop - 1, HOP_COLORS.length - 1)], color: '#fff' }" />
      ...
      <div v-else>
        <div style="font-size: 0.875rem; margin-bottom: 0.5rem;">
          <b>{{ store.blastRadius.totalAffected }}</b> files affected
          <span style="margin-left: 1rem; font-size: 0.8rem; color: var(--p-text-muted-color);">
            <span v-for="(color, i) in HOP_COLORS.slice(0, 4)" :key="i" style="margin-right: 0.75rem;">
              <span :style="{ display: 'inline-block', width: '10px', height: '10px', borderRadius: '50%', background: color, marginRight: '4px' }" />
              Hop {{ i + 1 }}{{ i === 3 ? '+' : '' }}
            </span>
          </span>
        </div>
        <VChart :option="treeChartOption" autoresize style="height: 400px; width: 100%;" />
      </div>
    </template>
  </Card>
</template>
