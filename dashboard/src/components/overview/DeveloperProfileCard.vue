...
const peakHoursChart = computed(() => {
  ...
  if (Array.isArray(raw)) {
    hours = raw.map((h: any) => ({ label: `${h.hour}h`, value: h.count }))
      ...
      <div v-else style="display: flex; flex-direction: column; gap: 0.75rem;">
        ...
        <div v-if="profile.preferredTools?.length">
          ...
          <div style="display: flex; gap: 0.25rem; flex-wrap: wrap; margin-top: 0.25rem;">
            <Chip v-for="t in profile.preferredTools.slice(0, 8)" :key="typeof t === 'string' ? t : t.tool" :label="typeof t === 'string' ? t : t.tool" style="font-size: 0.75rem;" />
          </div>
        </div>
      </div>
    </template>
