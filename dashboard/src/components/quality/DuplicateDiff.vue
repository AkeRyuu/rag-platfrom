...
const maxLines = computed(() => Math.max(linesA.value.length, linesB.value.length))

function lineClass(lineA: string | undefined, lineB: string | undefined): string {
    ...
    <template #title>
      <div style="display: flex; justify-content: space-between; align-items: center;">
        <div style="display: flex; align-items: center; gap: 0.5rem;">
          ...
          <Tag :value="`${(group.similarity * 100).toFixed(1)}% similar`" severity="info" />
        </div>
        <Button icon="pi pi-times" text size="small" @click="emit('close')" />
      </div>
    </template>
    <template #content>
      <div v-if="!fileA?.content && !fileB?.content" style="color: var(--p-text-muted-color); font-size: 0.85rem; padding: 1rem;">
        No code content available for comparison. The API returned file references without inline content.
        <div style="margin-top: 0.75rem; display: flex; flex-direction: column; gap: 0.25rem;">
          <div v-for="f in group.files" :key="f.file" style="font-family: monospace; font-size: 0.8rem;">{{ f.file }}</div>
        </div>
      </div>
      ...
      <div v-else class="diff-container">
        ...
        <div class="diff-header">
          <div class="diff-col">{{ fileA?.file?.split('/').pop() || 'File A' }}</div>
          <div class="diff-col">{{ fileB?.file?.split('/').pop() || 'File B' }}</div>
        </div>
...
.diff-changed { background: rgba(234, 179, 8, 0.1); }
</style>
