...
const languageChart = computed(() => {
  ...
  return {
    ...
    yAxis: { type: 'value' },
    series: [{ type: 'bar', data: entries.map(e => e[1]), itemStyle: { color: '#3B82F6' } }],
  }
})
</script>

<template>
  <div v-if="!analytics" style="padding: 1rem; color: var(--p-text-muted-color); font-size: 0.875rem;">
    No analytics available.
  </div>
  <div v-else style="display: flex; flex-direction: column; gap: 0.75rem; padding-top: 0.5rem; font-size: 0.875rem;">
    <div><b>Vectors:</b> {{ analytics.vectors?.toLocaleString() }}</div>
    <div><b>Segments:</b> {{ analytics.segments }}</div>
    <div v-if="analytics.diskUsageMb != null"><b>Disk Usage:</b> {{ analytics.diskUsageMb.toFixed(1) }} MB</div>

    <div v-if="languageChart">
      <b>Language Breakdown:</b>
      <VChart :option="languageChart" autoresize style="height: 200px; width: 100%; margin-top: 0.5rem;" />
    </div>
  </div>
</template>
