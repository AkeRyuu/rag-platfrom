...
import type { CollectionInfo, IndexStatus, VectorParams, Snapshot, CollectionAnalytics as AnalyticsType } from '@/types/collections'
...
<template>
  <Card>
    ...
    <template #content>
      <IndexingProgress v-if="indexStatus" :status="indexStatus" style="margin-bottom: 0.75rem;" />
      ...
      <Tabs value="info">
        ...
        <TabPanels>
          <TabPanel value="info">
            <div style="display: flex; flex-direction: column; gap: 0.75rem; font-size: 0.875rem; padding-top: 0.5rem;">
              ...
              <div v-if="isNamedVectors">
                <b>Named Vectors:</b>
                <div v-for="nv in namedVectors" :key="nv.name" style="padding-left: 0.75rem; margin-top: 0.25rem;">
                  <b>{{ nv.name }}:</b> size={{ nv.size }}, distance={{ nv.distance }}
                </div>
              </div>
              ...
              <div style="display: flex; gap: 0.5rem; flex-wrap: wrap; margin-top: 0.5rem;">
                <Button label="Reindex" icon="pi pi-refresh" size="small" severity="info" outlined @click="emit('reindex')" />
                <Button label="Snapshot" icon="pi pi-camera" size="small" severity="secondary" outlined @click="emit('snapshot')" />
                <Button label="Clear" icon="pi pi-eraser" size="small" severity="warn" outlined @click="emit('clear')" />
                <Button label="Delete" icon="pi pi-trash" size="small" severity="danger" outlined @click="emit('delete')" />
              </div>
            </div>
          </TabPanel>
          ...
          <TabPanel value="snapshots">
            <div v-if="!snapshots?.length" style="padding: 1rem; color: var(--p-text-muted-color); font-size: 0.875rem;">
              No snapshots yet.
            </div>
            <div v-else style="display: flex; flex-direction: column; gap: 0.5rem; padding-top: 0.5rem;">
              <div v-for="snap in snapshots" :key="snap.name" style="display: flex; justify-content: space-between; font-size: 0.875rem; padding: 0.5rem; background: var(--p-surface-50); border-radius: 6px;">
                <span>{{ snap.name }}</span>
                <span style="color: var(--p-text-muted-color);">{{ (snap.size / 1024 / 1024).toFixed(1) }} MB</span>
              </div>
            </div>
          </TabPanel>
        </TabPanels>
