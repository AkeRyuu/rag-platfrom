  ...
  <Card v-if="store.selectedResult" style="height: 100%;">
    <template #title>
      <div style="display: flex; justify-content: space-between; align-items: center;">
        ...
        <Button icon="pi pi-times" text size="small" @click="store.selectedResult = null" />
      </div>
    </template>
    <template #content>
      <div style="display: flex; flex-direction: column; gap: 0.5rem;">
        <div style="font-family: monospace; font-size: 0.8rem; color: var(--p-primary-color); word-break: break-all;">
          {{ store.selectedResult.file }}
          <span v-if="store.selectedResult.line" style="color: var(--p-text-muted-color);">:{{ store.selectedResult.line }}</span>
        </div>
        <div style="display: flex; gap: 0.25rem;">
          <Tag v-if="store.selectedResult.language" :value="store.selectedResult.language" severity="info" style="font-size: 0.7rem;" />
          <Tag v-if="store.selectedResult.layer" :value="store.selectedResult.layer" severity="secondary" style="font-size: 0.7rem;" />
          <Tag :value="`Score: ${(store.selectedResult.score * 100).toFixed(0)}%`" severity="success" style="font-size: 0.7rem;" />
        </div>
...
.hljs-title { color: #EAB308; }
.hljs-type { color: #EC4899; }
.hljs-attr { color: #F59E0B; }
