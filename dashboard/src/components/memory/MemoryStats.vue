...
<template>
  <div style="display: flex; gap: 0.5rem; flex-wrap: wrap; align-items: center;">
    ...
    <template v-if="stats?.byType">
      <span v-for="(count, type) in stats.byType" :key="type" style="display: inline-flex; align-items: center; gap: 2px;">
        <Tag :severity="(typeColors[type] as any) || 'secondary'" :value="`${type}: ${count}`" />
        <Button
          v-if="count > 0"
          icon="pi pi-trash"
          severity="danger"
