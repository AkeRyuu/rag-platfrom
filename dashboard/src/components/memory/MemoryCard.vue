    ...
    <template #content>
      <div style="display: flex; flex-direction: column; gap: 0.5rem; height: 100%;">
        ...
        <div style="flex: 1; font-size: 0.875rem; line-height: 1.4; white-space: pre-wrap; word-break: break-word;">
          {{ expanded || !needsTruncation ? memory.content : memory.content.slice(0, 200) + '...' }}
          <a v-if="needsTruncation" href="#" @click.prevent="expanded = !expanded" style="font-size: 0.8rem; margin-left: 0.25rem;">
            {{ expanded ? 'less' : 'more' }}
          </a>
        </div>

        <div v-if="memory.tags?.length" style="display: flex; gap: 0.25rem; flex-wrap: wrap;">
          <Chip v-for="tag in memory.tags" :key="tag" :label="tag" style="font-size: 0.75rem;" />
        </div>

        <div style="display: flex; justify-content: flex-end;">
          <Button icon="pi pi-trash" severity="danger" text size="small" @click="emit('delete', memory.id)" />
        </div>
      </div>
    </template>
  </Card>
