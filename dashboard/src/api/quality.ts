...
function getDefaultCollection(): string {
  const project = localStorage.getItem('rag_project') || import.meta.env.VITE_DEFAULT_PROJECT || 'rag'
  return `${project}_code`
}

export async function fetchDuplicates(threshold = 0.9, limit = 50): Promise<DuplicateGroup[]> {
  const collection = getDefaultCollection()
  const { data } = await client.post('/api/duplicates', { collection, threshold, limit })
...
export async function fetchClusters(seedIds?: string[], limit = 20): Promise<CodeCluster[]> {
  const collection = getDefaultCollection()
  ...
  if (!ids || ids.length === 0) {
    try {
      const { data: scrollData } = await client.get(`/api/collections/${collection}/scroll`, {
  ...
  const { data } = await client.post('/api/clusters', { collection, seedIds: ids, limit })
