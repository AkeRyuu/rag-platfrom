...
function getProjectName(): string {
  return localStorage.getItem('rag_project') || import.meta.env.VITE_DEFAULT_PROJECT || 'rag'
}
...
export async function searchHybrid(query: string, options: {
  limit?: number
  language?: string
  layer?: string
...
export async function searchSemantic(query: string, options: {
  limit?: number
  language?: string
  layer?: string
...
export async function searchSymbol(symbol: string, options: {
  kind?: string
  limit?: number
} = {}): Promise<{ results: SearchResult[] }> {
  ...
  const results: SearchResult[] = (data.symbols || data.results || []).map((s: any) => ({
...
export async function searchGraph(query: string, options: {
  limit?: number
  expandHops?: number
} = {}): Promise<{ results: SearchResult[] }> {
