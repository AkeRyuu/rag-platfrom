...
import type { GraphNode, GraphLink, BlastRadiusResult, NodeInspectorData } from '@/types/graph'
...
function normalizeEdges(data: any): { nodes: GraphNode[]; links: GraphLink[] } {
  ...
  let rawEdges: Array<{ source: string; target: string; type?: string }> = []
  ...
  // Deduplicate edges by unique file pairs
  ...
  for (const edge of uniqueEdges) {
    if (!seen.has(edge.source)) {
      nodes.push({ id: edge.source, name: edge.source.split('/').pop() || edge.source, type: 'file', connectionCount: connectionCount.get(edge.source) || 1 })
      seen.add(edge.source)
    }
    if (!seen.has(edge.target)) {
      nodes.push({ id: edge.target, name: edge.target.split('/').pop() || edge.target, type: 'file', connectionCount: connectionCount.get(edge.target) || 1 })
      seen.add(edge.target)
    }
    links.push({ source: edge.source, target: edge.target, type: edge.type || 'imports' })
  }
...
export async function fetchBlastRadius(file: string, maxDepth = 3): Promise<BlastRadiusResult> {
  const { data } = await client.post('/api/graph/blast-radius', { files: [file], maxDepth })
  return {
...
export async function fetchFileExports(file: string): Promise<NodeInspectorData> {
  try {
    ...
    try {
      ...
      const deps = (depsRes.data.dependencies || []).map((d: any) => d.toFile).filter(Boolean)
      const revDeps = (revRes.data.dependents || []).map((d: any) => d.fromFile).filter(Boolean)
