  ...
  function powerIteration(data: number[][], deflated?: number[]): number[] {
    ...
    for (let iter = 0; iter < 50; iter++) {
      ...
      const projected = data.map(row => {
        ...
        if (deflated) d -= deflated.reduce((s, x, i) => s + x * vec[i], 0) * row.reduce((s, x, i) => s + x * deflated[i], 0)
        return d
      })
...
export const useVectorsStore = defineStore('vectors', () => {
  ...
  async function loadAndProject(collectionName: string) {
    ...
    try {
      const result = await scrollVectors(collectionName, maxPoints.value, undefined, true)
      points.value = result.points

      // Filter points that have vectors
      const withVectors = result.points.filter(p => p.vector && p.vector.length > 0)
  ...
  return {
    points, projected, loading, computing, error, collection, selectedPoint, colorBy, maxPoints,
    loadAndProject,
  }
})
