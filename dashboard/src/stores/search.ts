...
import { ref } from 'vue'
import { searchHybrid, searchSemantic, searchSymbol, searchGraph } from '@/api/search'
...
export const useSearchStore = defineStore('search', () => {
  ...
  function loadRecent(): string[] {
    try {
      return JSON.parse(localStorage.getItem(RECENT_SEARCHES_KEY) || '[]').slice(0, 10)
    } catch { return [] }
  }
  ...
  return {
    query, mode, results, meta, loading, error, selectedResult,
    languageFilter, layerFilter, scoreThreshold, recentSearches,
    search, clearResults,
  }
})
