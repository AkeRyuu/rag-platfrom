...
export const useAppStore = defineStore('app', () => {
  ...
  const apiUrl = ref(localStorage.getItem('rag_api_url') || import.meta.env.VITE_RAG_API_URL || '')
  ...
  const autoRefreshInterval = ref(Number(localStorage.getItem('rag_refresh_interval') || '0'))
  ...
  watch(autoRefreshInterval, (v) => localStorage.setItem('rag_refresh_interval', String(v)))
  ...
  return {
    currentProject, apiKey, apiUrl, isConnected, healthError,
    isDark, autoRefreshInterval, isSidebarOpen,
    checkHealth, initDarkMode, clearAllStorage,
  }
})
