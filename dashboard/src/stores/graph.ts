...
import { ref } from 'vue'
import { fetchDependencies, fetchDependents, fetchBlastRadius, fetchFileExports } from '@/api/graph'
import type { GraphNode, GraphLink, BlastRadiusResult, LayoutMode, EdgeTypeFilter, NodeInspectorData } from '@/types/graph'
...
export const useGraphStore = defineStore('graph', () => {
  ...
  async function search() {
    ...
    try {
      ...
      if (mode.value === 'both') {
        ...
        // Deduplicate links
        const linkSet = new Set(links.value.map(l => `${l.source}->${l.target}`))
  ...
  return {
    ...
    layoutMode, edgeTypeFilter, selectedNode, inspectorData, inspectorLoading, filePatternFilter,
    blastRadius, blastLoading, blastViewMode,
    search, selectNode, clearSelection, analyzeBlastRadius,
  }
})
